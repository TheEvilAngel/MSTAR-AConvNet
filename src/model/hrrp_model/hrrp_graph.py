...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x, adj):
        x = x.transpose(1, 2) # [1, 500, 16]
        support = torch.matmul(adj, x) # [1, 500, 16]
        out = torch.matmul(support, self.weight1) + torch.matmul(x, self.weight2) + self.bias # [1, 500, 16]
        out = out.transpose(1, 2) # [1, 16, 500]
        return out
...
class HRRPGraphNet(nn.Module):
    def __init__(self, input_length, output_length):
        ...
        self.attention = nn.Linear(input_length, 1)
        xavier_uniform_(self.attention.weight)
        zeros_(self.attention.bias)
        self.fc = nn.Linear(input_length, output_length)  # Adjust input features if necessary
    ...
    def construct_adj_matrix(self, x, distance_matrix):
        ...
        adj_matrix = torch.bmm(x_transposed, x) # [1, 500, 500]
        distance_matrix = distance_matrix.unsqueeze(0).expand(batch_size, -1, -1) # 1, 500, 500
        adj_matrix = adj_matrix * distance_matrix
        return adj_matrix

    def forward(self, x, distance_matrix):
        adj = self.construct_adj_matrix(x, distance_matrix)
        x = self.leakyrelu(self.bn1(self.conv1(x))) # [1, 16, 500]
        x = self.graph_conv(x, adj) # [1, 16, 500]
...
if __name__ == "__main__":
    model = HRRPGraphNet(num_classes=3)
    x = torch.randn(1, 1, 500)
    distance_matrix = torch.randn(500, 500)
    output = model(x, distance_matrix)
